<%@ Page Language="C#" MasterPageFile="/umbraco/masterpages/umbracoPage.Master" AutoEventWireup="true" CodeBehind="UploadMedia.aspx.cs" Inherits="uMediaPicker.umbraco.plugins.uMediaPicker.UploadMedia" %>
...
<asp:Content ID="Content" ContentPlaceHolderID="body" runat="server">
    ...
    <div class="topDiv" style="width:100%;float:left;height:30px;margin-bottom:5px;">
        <p style="font-size:14px;">1. Choose Folder</p>
    </div>
        ...
        <div>
            <div class="players">
                <asp:Panel ID="pnlUpload" runat="server">
                    <span class="selectedFolder" style="font-size:12px">Selected folder: Root</span>
                    <p style="font-size:14px;">2. Choose File to upload</p>
                    <asp:FileUpload ID="uploadMedia" CssClass="fileUploadBox" runat="server" />
                    <asp:Button ID="btnUpload" Text="Upload" runat="server" 
                        onclick="btnUpload_Click" />
                    <asp:HiddenField ID="hdnSelectedFolder" Value="-1" runat="server" />
                </asp:Panel>
    ...
    <script type="text/javascript">
        ...
        $(document).ready(function () {
            ...
            $("#mediaTreePicker").fileTree(
                {
                    root: '-1',
                    script: "/umbraco/plugins/uMediaPicker/GetMediaChildren.aspx?folderOnly=true",
                    multiFolder: true,
                    expandSpeed: 1000,
                    collapseSpeed: 1000
                },
            ...
            $("#jquery_jplayer_2").jPlayer({
                ready: function () {
                    $(this).jPlayer("setMedia", {
                        m4v: "/media/31/big_buck_bunny_trailer.m4v",
            ...
            if($("#<%= pnlPreviews.ClientID %>").length <= 0)
            {
                $(".previewDetails").hide();
                $("#mediaTreeButtonOK").hide()
                
            }
            else {
                $(".topDiv").hide();
                $("#mediaTreePicker").hide();
                setTimeout(function() { previewVideo(previewPath); }, 800);
            }
        });

function folderCallback(directory) {
    var dirName = $(directory).text();
    $(".selectedFolder").html("Selected folder: " + dirName + "&nbsp;<a href='#' onclick='javascript:ResetFolder();return false;'>Reset</a>");
    $("#<%= hdnSelectedFolder.ClientID %>").val($(directory).attr("rel"));
}

function ResetFolder() {
    $(".selectedFolder").text("Selected folder: Root");
    $("#<%= hdnSelectedFolder.ClientID %>").val("-1");
}
...
function previewVideo(path)
{
    ...
    $("#currentMediaName").text(path.split('/')[path.split('/').length - 1]);
